...
from db import TaskTable, GroupTable, new_session
...
class Repository:
    ...
    async def get_group(group_id: int, base_session=None):
        async with transaction(base_session) as session:
            statement = (
                select(GroupTable.id, GroupTable.name, GroupTable.created_at, TaskTable.status)
            ...
            async for row in result:
                ...
                status = row.status["state"] if row.status else "PREPARING"
                if status == "FAILED":
    ...
    async def get_working_tasks(base_session=None):
        async with transaction(base_session) as session:
            statement = (
                select(TaskTable.id)
                .where(TaskTable.status["state"].as_string().notin_(["SUCCESS", "FAILED"]))
