...
async def check_status(task):
    ...
    while True:
        ...
        task = await Repository.update_task(task.id, None, status=status)
...
@celery.task(soft_time_limit=60 * 60)
    ...
    try:
        ...
        async def collect_kpt_async(id):
            ...
            if not task.task_id:
                ...
                task = await Repository.update_task(task.id, None, task_id=task_id)
        ...
        asyncio.run(Repository.update_task(id, None, status={'state': 'CANCELLED'}))
    except Exception as e:
        asyncio.run(Repository.update_task(id, None, status={'state': 'FAILED', 'error': str(e)}))
        raise e
