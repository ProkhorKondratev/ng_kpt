...
class UploadFile {
    constructor(file, handler = handler) {
    ...
    initListeners() {
        const checkbox = this._baseEl.querySelector("#accept");
        checkbox.onchange = () => {
            this._accepted = checkbox.checked;
        };
    ...
    remove(fromDir = false) {
        this._baseEl.remove();
        if (!fromDir) this.handler.files = this.handler.files.filter((file) => file.name !== this.name);
    }
}

class UploadDirectory {
    constructor(directory, handler) {
        this.handler = handler;
    ...
    remove() {
        this.files.forEach((file) => file.remove(true));
        this._baseEl.remove();
        this.handler.directories = this.handler.directories.filter((directory) => directory.name !== this.name);
    }
}
...
export class UploadHandler {
    ...
    renderForm() {
        ...
        formEl.innerHTML = `
            <form class="ms-form">
                ...
                <div class="ms-form-check">
                    <div class="ms-form-check__body">
                        <label for="save_default_crs">Не трансформировать в WGS84:</label>
                        <input type="checkbox" name="save_default_crs" id="save_default_crs" class="ms-checkbox">
                    </div>
                    <p class="ms-form-info">Оставить объекты в исходной системе координат без их трансформации в WGS84</p>
                </div>
                <div class="ms-form-check">
                    <div class="ms-form-check__body">
        ...
        if (entry.isFile) {
            const filePromise = new Promise((resolve) => {
                if (typeof entry.file === "function") {
                    entry.file((file) => {
                        resolve(new UploadFile(file, this));
                    });
                } else {
                    resolve(new UploadFile(entry.file, this));
                }
            });
    ...
    renderDirectories() {
        ...
        this.directories.forEach((directory) => {
            files.appendChild(directory.render());
        });
    ...
    checkFile(file) {
        if (file.file.size >= 100 * 1024 && file.file.name.toLowerCase().endsWith(".zip")) {
            file.accepted = true;
        }
    }
